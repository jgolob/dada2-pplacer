...
def main():
    ...
    args_parser.add_argument('--map', '-m',
                             help="Write pplacer-style mapping of otu to specimen",
                             type=argparse.FileType('w'))
    args_parser.add_argument('--weights', '-w',
                             help="Write pplacer-style weights of otu by specimen",
                             type=argparse.FileType('w'))
    ...
    # Generate OTU labels for each sequence variant,
    # and create a dictionary that maps sequence-variant to otu_id...
    ...
    # from each OTU to be it's champion for guppy.
    # To do so, we will go through each column, find the max count for that OTU,
    # and use that specimen as the champion
    max_spec_for_otu = {otu_id: spec for otu_id, spec in seqtab.apply(lambda c: c.idxmax()).items()}

    if out_otu_seqs_h:
        # Write out the sequences in fasta format, using the otu-id's generated above as an ID
        for otu_id, seq in otu_num_to_seq.items():
    ...
    if map_writer or weights_writer:
        for spec, row in seqtab.iterrows():
            ...
            for otu_id, count in row_nonzero.items():
                if map_writer:
                    map_writer.writerow([str(otu_id) + ":" + str(spec), spec])
                if weights_writer:
                    weights_writer.writerow([
                        otu_id + ":" + max_spec_for_otu[otu_id],
                        str(otu_id) + ":" + str(spec),
                        count])
